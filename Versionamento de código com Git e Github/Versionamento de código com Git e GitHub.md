...
- Ctrl + L : Gera varios espaços vazios e rola a barra de rolagem, de forma que fique o endereço de trabalho fique no topo da tela do terminal;
...
Anteriormente era possível realizar autenticações de repositórios privados entre as plataformas Git/GitHub, utilizando o email e senha cadastrados no, mas desde 13 de agosto de 2021, essa forma de autenticação foi removida, sendo necessário a criação de chaves token para autenticar uma alteração.
...
> *__Por motivos de segurança, após sair da tela onde se encontra a chave token criada, a mesma não poderá mais ser visualizada, recomenda-se a utilização imediata__*.
...
1. `git clone` + URL do repositório que se deseja clonar + Enter. Está URL está disponivel na página principal do repositório, clicando em `<> Code`, seleciona-se a aba `HTTPS` e copia-se a URL logo abaixo;
...
Uma outra possibilidade de autenticação de aolicitações no Git é utilizando uma chave do tipo SSH ( Secure Shell). 
...
>Antes de criar esse tipo de chave, podemos verificar se há alguma chave configurada, podendo ser ela, pública ou privada. 
